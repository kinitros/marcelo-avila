import { Routes, Route } from "react-router-dom";
import { Header } from "@/sections/Header";
import { Hero } from "@/sections/Hero";
import { Footer } from "@/sections/Footer";
import { RegistroMarcasPage } from "@/pages/RegistroMarcasPage";
import { DireitoSaudePage } from "@/pages/DireitoSaudePage";

const WhatsAppButton = () => (
  <a
    href="https://wa.me/5521972728787?text=Olá! Gostaria de mais informações sobre os serviços do escritório."
    target="_blank"
    rel="noopener noreferrer"
    aria-label="Fale conosco pelo WhatsApp"
    className="fixed bottom-5 right-5 z-[9999] flex items-center justify-center w-14 h-14 bg-[#25d366] rounded-full shadow-[0_4px_12px_rgba(37,211,102,0.4)] hover:bg-[#20bd5a] hover:scale-110 transition-all duration-200"
  >
    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492a.5.5 0 00.613.613l4.458-1.495A11.952 11.952 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-2.352 0-4.55-.764-6.325-2.055l-.44-.334-2.634.883.883-2.634-.334-.44A9.935 9.935 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
  </a>
);

export const App = () => {
  return (
    <body className="text-zinc-800 text-base not-italic normal-nums font-normal accent-auto bg-white box-border caret-transparent block tracking-[normal] leading-6 list-outside list-disc overflow-x-clip pointer-events-auto text-start indent-[0px] normal-case visible border-separate font-apple_system">
      <Header />
      <Routes>
        <Route path="/" element={<Hero />} />
        <Route path="/registro-de-marcas" element={<RegistroMarcasPage />} />
        <Route path="/direito-saude" element={<DireitoSaudePage />} />
      </Routes>
      <Footer />
      <WhatsAppButton />
    </body>
  );
};
